...
async def send_welcome(message: types.Message):
    ...
    await message.answer(
        "Приветствую! Я бот, который поможет тебе не пропустить олимпиады по важным для тебя предметам! Введи команду /help, чтобы подробнее ознакомиться с моим функционалом",
        reply_markup=reply.default_user_markup)
...
async def takeolimpyad(message: types.Message, state: UserParams.Setolympiad):
    ...
    await message.answer(f"Выберете количество олимпиад, которые получите по запросу <b>{text}</b>:",
                         reply_markup=inline.numbers_1_8)
    await state.finish()
...
async def take_number_1_8(call: types.CallbackQuery):
    ...
    for olymp in olymps:
        await call.message.answer(f'• <b>Ссылка:</b> <i>{olymp.link}</i>\n'
